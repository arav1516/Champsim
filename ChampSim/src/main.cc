...
#include <bits/stdc++.h>
using namespace std;
std::map<int,int> eviction_count_map;
vector<pair<int,int>> sorted_eviction_count_vector;
...
void build_eviction_count_map_cache(CACHE* cache){
    for(int i=0;i<2048;i++){
        cache->eviction_count_map_cache[i]=0;
    }
}
void build_map()
{
  for(int i=0;i<2048;i++){
    eviction_count_map[i]=0;
  }
}
void build_map_cache(CACHE* cache)
{   int counting=0;
    //cout<<cache->sorted_eviction_count_vector_cache.size()<<endl;
    for(auto it = cache->sorted_eviction_count_vector_cache.begin(); it != cache->sorted_eviction_count_vector_cache.end(); ++it){
        cache->sorted_sets_map[counting]=it->first;
        counting++;
    }
    //cout<<counting<<endl;
}
// print_map(&uncore.LLC);
void print_map()
{    
    for(auto& it : eviction_count_map){
        cout<<it.first<<" "<<it.second<<endl;
    }
}
void print_map_cache(CACHE* cache){
    for(auto& it : cache->sorted_eviction_count_vector_cache){
    cout<<it.first<<" "<<it.second<<endl;
}
}
void print_sorted_sets_map(CACHE* cache){
    for(auto& it : cache->sorted_sets_map){
        cout<<it.first<<" "<<it.second<<endl;
    }
}
void update_map(CACHE *cache)
{   
    for(int i=0;i<2048;i++){
        eviction_count_map[i]=cache->eviction_count_map_cache[i];
    }
}

void print_total_evictions(CACHE* cache)
{
    cout<<cache->total_evictions<<endl;
}

void initialize_total_evictions(CACHE* cache)
{
...
{
    for(auto& it : eviction_count_map){
        sorted_eviction_count_vector.push_back(it);
    }
    sort(sorted_eviction_count_vector.begin(),sorted_eviction_count_vector.end(),cmp);
    /*for(auto& it : temporary_sorting_vector){
        eviction_count_map[it.first]=it.second;
    }*/

}
void sort_map_cache(CACHE* cache)
{   cache->sorted_eviction_count_vector_cache.clear();
    for(auto& it : cache->eviction_count_map_cache){
        cache->sorted_eviction_count_vector_cache.push_back(it);
    }
    sort(cache->sorted_eviction_count_vector_cache.begin(),cache->sorted_eviction_count_vector_cache.end(),cmp);
}
...
int main(int argc, char** argv)
{
    ...
    //initialize the build_map
    //build_map();
    build_eviction_count_map_cache(&uncore.LLC);
    initialize_total_evictions(&uncore.LLC);
    ...
    for (int i=0; i<argc; i++) {
        if (found_traces)
        {
            ...
            //cout<<"The trace file is "<<ooo_cpu[count_traces].trace_file <<endl;
    ...
    sort_map_cache(&uncore.LLC);
    //print_map_cache(&uncore.LLC);
    build_map_cache(&uncore.LLC);

    // simulation entry point
    start_time = time(NULL);
    uint8_t run_simulation = 1;
    uint32_t instruction_count=0;
    uint32_t last_instruction_count=0;
    while (run_simulation) {
	      ...
	      if ((ooo_cpu[i].IFETCH_BUFFER.occupancy < ooo_cpu[i].IFETCH_BUFFER.SIZE) && (ooo_cpu[i].fetch_stall == 0))
		{
          ...
          instruction_count++;
            ...
            if(warmup_complete[i] == 1){ //Using this I am sorting the vector having all the sets eviction details every 10000 instructions executed.
                if (instruction_count-last_instruction_count>=10000){
                    sort_map_cache(&uncore.LLC);
                    build_map_cache(&uncore.LLC);
                    //cout<<instruction_count<<endl;
                    uncore.LLC.mapping_constant=0;
    ...
    //cout<<"Printint the sorted_sets_map the second time"<<endl;
    //print_sorted_sets_map(&uncore.LLC);
    uint64_t elapsed_second = (uint64_t)(time(NULL) - start_time),
             elapsed_minute = elapsed_second / 60,
             elapsed_hour = elapsed_minute / 60;
    elapsed_minute -= elapsed_hour*60;
    ...
    print_dram_stats();
    print_branch_stats();
    //update_map(&uncore.LLC);
    //sort_map();
    //sort_map_cache(&uncore.LLC);
    //print_map_cache(&uncore.LLC);
    //print_map();
    //print_map_cache(&uncore.LLC);
    //print_total_evictions(&uncore.LLC);
#endif

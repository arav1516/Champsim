//only find_victim and get_set have been heavily modified look into that.
#include "cache.h"
#include "set.h"
#include <algorithm>
...
void CACHE::handle_fill() 
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        auto it = mapping_set.find(MSHR.entry[mshr_index].address);
        if (it!=mapping_set.end()){
          set=mapping_set[MSHR.entry[mshr_index].address];
        }
        else{
        ...
        if (cache_type == IS_LLC) {
            if(warmup_complete[fill_cpu]){
              ...
              eviction_count_map_cache[set]=eviction_count_map_cache[set]+1;
              }
            way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
            auto it = already_there_mapping_set.find(block[set][way].address);
            if (it!=already_there_mapping_set.end()){
              already_there_mapping_set.erase(it);
            }
            else{
              auto it = mapping_set.find(block[set][way].address);
              if (it!=mapping_set.end()){
                mapping_set.erase(it);
              }
            }
        }//This was added yesterday due to an error.
            //cout<<"The address we are bound to use"<<MSHR.entry[mshr_index].address<<endl;
            //cout<<"The adress is "<<block[set][way].address<<endl;
            //cout<<"The full adress is "<<block[set][way].full_addr<<endl;
            /*if(way==-1){
              for (auto it =eviction_count_map_cache.begin();it!=eviction_count_map_cache.end();++it){
                set = it->first;
                break;
              }
              mapping_set[MSHR.entry[mshr_index].address]=set;
              way =llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type); 
            }*/
...
{   
    /*if(cache_type==IS_LLC){
      auto it = eviction_count_map_cache.find((uint32_t) (address & ((1 << lg2(NUM_SET)) - 1)));
      if (it != eviction_count_map_cache.end()) {
          eviction_count_map_cache[(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1))]++;
      } else {
          eviction_count_map_cache[(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1))]=1;
      }
    }*/
      if(check_warmup==1 && cache_type==IS_LLC){
      auto it=already_there_mapping_set.find(address);
      if(it!=already_there_mapping_set.end()){
        return already_there_mapping_set[address];
      }
      else{
      auto it=mapping_set.find(address);
      if (it!=mapping_set.end()){
        return mapping_set[address];
      }
      else{
         int temporary=mapping_constant%512;
         mapping_set[address]=sorted_sets_map[temporary];
         mapping_constant=mapping_constant+1;//I will also be ensuring to get this mapping_constant to 0 every time I sort the vector again.
         return sorted_sets_map[temporary];
        /*int current_set_index;
        uint32_t original_set=(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1));
        for(int i=0;i<2048;i++){
          if(sorted_sets_map[i]==original_set)
          {
            current_set_index=i;
            break;
          }
        }
        mapping_set[address]=sorted_sets_map[2047-current_set_index];
        return sorted_sets_map[2047-current_set_index];*/
      }
      ...
      if(cache_type==IS_LLC){
        auto it=already_there_mapping_set.find(address);
        if(it==already_there_mapping_set.end()){
          already_there_mapping_set[address]=(uint32_t) (address & ((1 << lg2(NUM_SET)) - 1));
        }
      }
...
/*bool CACHE::cmp(pair<int,int>& a,pair<int,int>& b)
{
    return a.second<b.second;
}
void CACHE::sorted_eviction_count_vector()
{
      for(auto& it : eviction_count_map_cache){
        sorted_eviction_count_vector_cache.push_back(it);
    }
    std::sort(sorted_eviction_count_vector_cache.begin(),sorted_eviction_count_vector_cache.end(),cmp);
}
*/